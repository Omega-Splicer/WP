...
namespace OmegaSplicer.Common
{
    public class BLEManager
    {
        ...
        public GattDeviceService selectService(string service)
        {
            if (currentDevice != null)
               return selectedService = currentDevice.GetGattService(new Guid(service));
        ...
        public async Task<GattCommunicationStatus> write(string UUID, byte[] data)
        {
            GattCharacteristic characteristic = selectedService.GetCharacteristics(new Guid(UUID))[CHARACTERISTIC_INDEX];
            return await characteristic.WriteValueAsync(data.AsBuffer(), GattWriteOption.WriteWithResponse);
        }
        /// <summary>  
        ...
        static public BLEManager GetInstance()
        {
            if (instance == null)
