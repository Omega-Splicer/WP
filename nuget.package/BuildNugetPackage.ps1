...
$projectPath = Join-Path $root "..\OmegaSplicer"
$slnPath = Join-Path $projectPath "..\OmegaSplicer.sln"
...
try {
  Set-Content -Encoding ASCII $(Join-Path $projectPath "OmegaSplicer_hash.txt") $commitSha
  ...
  Pop-Location
  Set-Content -Encoding ASCII $(Join-Path $projectPath "OmegaSplicer_hash.txt") "unknown"
}
